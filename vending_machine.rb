...
class VendingMachine
  ...
  attr_reader :total, :sale_amount, :stocks, :unstock
  ...
  def initialize
    ...
    @stocks.push(preparation) if preparation.validate_class && preparation.validate_unique && preparation.kind_of?(Drink)
    @unstock = []
  end
  ...
  def drink_menu
    ...
    unless @stocks.empty?
      @stocks.each_with_index do |stock, idx|
        ...
        unstock_drink(drink.name) if ! stock.drinks.empty?
        puts "[#{idx}]:#{drink.name}#{drink.price}円" if ! stock.drinks.empty?
        puts "[#{idx}]:#{@unstock[idx]}は、ただいま品切れ中です。" if stock.drinks.empty?
      end
    end
    puts "-----------------------------------------"
    puts "[x]払い戻し"
  ...
  def unstock_drink(name)
    @unstock << name unless @unstock.include?(name)
  end
  ...
  def store(drink, num)
   ...
   #if drink == Drink.cola.name || @drinks.push(drink).values_at  == Drink.redbull.name || @drinks.push(drink).values_at  == Drink.water.name
   #if @drinks.push(drink)  == Drink.cola.name ||   Drink.redbull.name || Drink.water.name
    stocks_delete
    @unstock.clear#clearを使うと配列の中身に要素があっても空にしてしまう
    @drinks = []
